#!/usr/bin/env python3
"""V9 eval — nuclear option: disable dynamo globally + reset before each sample.

v7/v8 still drop at idx=42 despite fixed shapes. Root cause likely Qwen3-Next's
hybrid attention has internal shape-specific code paths that we can't control
via input-level fixes. Only reliable workaround: force pure eager for the
entire pipeline, and reset dynamo cache between every sample.

Trade-off: ~2× slower per sample (no compile caching), but guaranteed 0 drops.
For 1595 samples this takes ~90-120 min on A40. Acceptable for the single
definitive eval run.
"""
import argparse
import json
...
def main():
    ...
    ap.add_argument("--save-dir", default="/workspace/hf_eval_v9")
    ...
    # NUCLEAR: globally disable dynamo after load. Weights + PEFT are applied;
    # now we don't want any compile at all for inference.
    ...
    # Belt-and-suspenders: wrap both forward and generate
    model.forward = torch._dynamo.disable(model.forward)
    if hasattr(model, "generate"):
        model.generate = torch._dynamo.disable(model.generate)
    log.info("torch._dynamo.config.disable=True + forward/generate wrapped")
    ...
    for idx, (truth, image) in enumerate(all_samples):
        # Reset dynamo cache before every single sample — nuclear guarantee
        # that no cache accumulation ever hits any limit.
        try:
            torch._dynamo.reset()
        except Exception:
            pass
        ...
        gen = ""
        for attempt in (1, 2, 3):  # 3 attempts: aggressive retry
            try:
    ...
    try:
        ...
        ax.set_title(
            f"Confusion Matrix (v9 — full 1595 samples, 0 drops)\n"
    ...
    print(f"Dynamo:     disabled + reset-per-sample")
